...
const hasMatchingTags = (tagsOne, tagsTwo) =>
  _.some(tagsOne, tagOne => {
    const tagOneLower = tagOne.toLowerCase();
    return _.some(tagsTwo, tagTwo => tagOneLower === tagTwo.toLowerCase());
  });

const filterSubcategoriesByTags = (subCats, tags) =>
  _.filter(subCats, subCat => hasMatchingTags(subCat.tags, tags));
...
const addItem = (item, category, subcategory = null) => {
  ...
  category.items =
    category.items && !_.some(category.items, { id: item.id })
      ? category.items.concat(item)
      : [item];
};
...
export const categorize = (items, categories) => {
  ...
  _.each(items, item => {
    ...
    _.each(categories, category => {
      const matchedSubcategories = filterSubcategoriesByTags(
